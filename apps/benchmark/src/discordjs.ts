...
client.once("ready", (readyClient) => {
  ...
  if (readyClient.ws.shards.size > 1) {
    console.log(`[SHARDING] ${readyClient.ws.shards.size} shards ready`);
  }

  console.log(`[GUILDS] Connected to ${readyClient.guilds.cache.size} guilds`);
...
async function main(): Promise<void> {
  try {
    ...
    await client.login(parsed?.DISCORD_TOKEN);
