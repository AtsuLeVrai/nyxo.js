...
export class CDN extends EventEmitter<CDNEvents> {
	...
	public customEmoji(
		emojiId: Snowflake,
		format: Exclude<ImageFormats, "json"> = "png",
		size?: Integer,
	): string {
	...
	public guildSplash(
		guildId: Snowflake,
		guildSplash: string,
		format: Exclude<ImageFormats, "gif" | "json"> = "png",
		size?: Integer,
	): string {
		const url = new URL(
	...
	public defaultUserAvatar(
		user: Pick<UserStructure, "discriminator" | "id">,
	): string {
		const hash =
	...
	public avatarDecoration(decoration: string, size?: Integer): string {
		const url = new URL(
			`avatar-decoration-presets/${decoration}.png`,
			this.baseUrl,
		);
		if (size) {
			url.searchParams.append("size", size.toString());
	...
	public applicationCover(
		...
		const url = new URL(
			`app-icons/${applicationId}/${cover}.${format}`,
			this.baseUrl,
		);
		if (size) {
			url.searchParams.append("size", size.toString());
	...
	public achievementIcon(
		...
		const url = new URL(
			`app-assets/${applicationId}/achievements/${achievementId}/icons/${icon}.${format}`,
			this.baseUrl,
		);
		if (size) {
			url.searchParams.append("size", size.toString());
	...
	public sticker(
		stickerId: Snowflake,
		format: Exclude<ImageFormats, "jpeg" | "jpg" | "webp"> = "png",
	): string {
	...
	public guildScheduledEventCover(
		...
		const url = new URL(
			`guild-events/${eventId}/${cover}.${format}`,
			this.baseUrl,
		);
		if (size) {
			url.searchParams.append("size", size.toString());
	...
	public guildMemberBanner(
		...
		const url = new URL(
			`guilds/${guildId}/users/${userId}/banners/${banner}.${format}`,
			this.baseUrl,
		);
		if (size) {
			url.searchParams.append("size", size.toString());
	...
	private validateIconFormat(hash: string, format: string): boolean {
		if (this.isAnimatedHash(hash) && format !== "gif") {
			return this.emit(
				"DEBUG",
				`You have the potential to use a gif format for the hash: ${hash}`,
			);
		}
		if (format === "gif" && !this.isAnimatedHash(hash)) {
