...
function fromArray(headers: string[]): Record<string, string | string[]> {
...
function normalizeHeaders(
  ...
  if (Array.isArray(headers)) {
    return fromArray(headers);
  }

  if (isIterable(headers)) {
    return fromIterable(headers);
  }
  ...
  return Object.entries(headers).reduce<Record<string, string>>(
    (acc, [key, value]) => {
      ...
      if (normalizedValue !== undefined) {
        acc[normalizeKey(key)] = normalizedValue;
      }
      return acc;
    },
    {},
...
export const HeaderHandler = {
  parse(headers: RawHeaders): ParsedHeaders {
    ...
    const normalizedHeaders = convertToStringRecord(rawHeaders);
  ...
  getValue(headers: RawHeaders, key: string): string | undefined {
    return this.parse(headers).headers[normalizeKey(key)];
  },
  ...
  has(headers: RawHeaders, key: string): boolean {
    return normalizeKey(key) in this.parse(headers).headers;
  },
} as const;
