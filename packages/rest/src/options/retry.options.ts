...
export const RetryOptions = z.object({
   ...
   * @default 5
   */
  maxRetries: z.number().nonnegative().int().min(0).default(5),

  /**
   * Maximum timeout in milliseconds between retry attempts
   * @default 30000
   */
  maxTimeout: z.number().nonnegative().int().min(0).default(30000),

  /**
   * Minimum timeout in milliseconds between retry attempts
   * @default 500
   */
  minTimeout: z.number().nonnegative().int().min(0).default(500),
   ...
   * @default 2
   */
  timeoutFactor: z.number().nonnegative().int().min(1).default(2),
