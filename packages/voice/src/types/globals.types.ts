 ...
 * Voice State
 *
 * Represents a user's voice connection status within a guild.
 * Includes information about channel, mute/deaf status, and
 * other properties of the voice connection.
 *
 * Voice States are announced when users join, leave, or move
 * between voice channels via Voice State Update events.
 *
 * @see {@link https://discord.com/developers/docs/events/gateway-events#voice-state-update}
 */
export interface VoiceStateEntity {
  /**
   * ID of the guild this voice state is for
   * Omitted in payloads from the Guild Create event
   */
  guild_id?: Snowflake;

  /**
   * ID of the channel this user is connected to
   * Null when the user is not connected to any channel
   */
  channel_id: Snowflake | null;

  /**
   * ID of the user this voice state is for
   * Identifies which user's voice connection this represents
   */
  user_id: Snowflake;

  /**
   * Guild member object for this voice state
   * Contains member information for context
   */
  member?: any; // GuildMemberEntity in full implementation

  /**
   * Session ID for this voice connection
   * Used for resuming connections and correlation with voice gateway
   */
  session_id: string;

  /**
   * Whether this user is deafened by the server
   * Server-deafened users cannot receive any voice data
   */
  deaf: boolean;

  /**
   * Whether this user is muted by the server
   * Server-muted users cannot transmit voice data
   */
  mute: boolean;

  /**
   * Whether this user has deafened themselves
   * Self-deafened users have chosen not to receive audio
   */
  self_deaf: boolean;

  /**
   * Whether this user has muted themselves
   * Self-muted users have chosen not to transmit audio
   */
  self_mute: boolean;

  /**
   * Whether this user is streaming using "Go Live"
   * Indicates screen sharing or application streaming
   */
  self_stream?: boolean;

  /**
   * Whether this user's camera is enabled
   * Indicates video transmission is active
   */
  self_video: boolean;

  /**
   * Whether this user is muted by the current user
   * Client-side muting for specific users
   */
  suppress: boolean;

  /**
   * ISO8601 timestamp of when this user requested to speak
   * Used for stage channels with audience/speaker separation
   */
 ...
 * Voice Server Update
 *
 * Event sent by the main gateway when a guild's voice server is updated.
 * Contains the information needed to establish a voice connection.
 *
 * This event is triggered when initially connecting to voice and when
...
export interface VoiceServerUpdateEntity {
   ...
   * Used for authentication with the voice WebSocket
   */
  token: string;
   ...
   * Null indicates the voice server is unavailable or being reallocated
   */
