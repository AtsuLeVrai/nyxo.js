...
export enum VoiceGatewayOpcode {
   ...
   *
   * **Direction:** Client → Server
   * **Purpose:** DAVE protocol MLS external sender configuration for E2EE.
   * **Protocol:** Part of Message Layer Security (MLS) group management.
   */
  DaveMlsExternalSender = 13,

  /**
   * DAVE MLS Commit.
   *
   * **Direction:** Client → Server
   * **Purpose:** DAVE protocol MLS commit message for group state changes.
   * **Use Case:** Updates encryption group when members join/leave.
   */
  DaveMlsCommit = 14,

  /**
   * DAVE MLS Announce Commit.
   *
   * **Direction:** Server → Client
   * **Purpose:** DAVE protocol MLS commit announcement to all participants.
   * **Result:** Synchronizes encryption state across all group members.
   */
  DaveMlsAnnounceCommit = 15,

  /**
   * DAVE MLS Proposals.
   *
   * **Direction:** Client → Server
   * **Purpose:** DAVE protocol MLS proposals for group membership changes.
   * **Use Case:** Proposes adding or removing members from E2EE group.
   */
  DaveMlsProposals = 16,
   ...
   *
   * **Direction:** Server → Client
   * **Purpose:** DAVE protocol invalid commit notification with recovery data.
   * **Recovery:** Provides new welcome message when commit processing fails.
   */
  DaveMlsInvalidCommitWelcome = 17,

  /**
   * DAVE Prepare Transition.
   *
   * **Direction:** Server → Client
   * **Purpose:** DAVE protocol transition preparation for protocol changes.
   * **Use Case:** Announces upcoming E2EE protocol version changes or downgrades.
   */
   ...
   *
   * **Direction:** Server → Client
   * **Purpose:** DAVE protocol transition execution signal.
   * **Result:** Triggers switch to new protocol state across all participants.
   */
   ...
   *
   * **Direction:** Client → Server
   * **Purpose:** DAVE protocol transition readiness confirmation.
   * **Coordination:** Indicates client is prepared for protocol transition.
   */
   ...
   *
   * **Direction:** Server → Client
   * **Purpose:** DAVE protocol epoch preparation for MLS group updates.
   * **Use Case:** Announces new MLS epoch with protocol version information.
   */
  DavePrepareEpoch = 24,
   ...
   *
   * **Direction:** Client → Server
   * **Purpose:** DAVE protocol MLS key package for group membership.
   * **Requirement:** Must be sent to join or rejoin an E2EE group.
   */
...
/**
 * DAVE Protocol Transition Types
 *
 * Enumeration of the types of protocol transitions that can occur during a call.
 * These transitions coordinate changes in encryption state across all participants.
 *
 * **Coordination:** All participants must be ready before transition execution.
 *
 * @see {@link https://daveprotocol.com/#protocol-transitions}
 */
export enum DaveTransitionType {
  /**
   * Downgrade from E2EE to non-E2EE
   *
   * **Trigger:** Non-E2EE client joins the call
   * **Effect:** All participants switch to transport encryption only
   * **Compatibility:** Ensures all clients can participate
   */
  Downgrade = "downgrade",

  /**
   * Upgrade from non-E2EE to E2EE
   *
   * **Trigger:** All clients support E2EE protocol
   * **Effect:** Establishes end-to-end encryption for all participants
   * **Security:** Enhanced privacy and security for media transmission
   */
  Upgrade = "upgrade",
   ...
   *
   * **Trigger:** Server negotiates new protocol version
   * **Effect:** All participants switch to new DAVE protocol version
   * **Purpose:** Feature updates and security improvements
   */
  ProtocolVersionChange = "protocol_version_change",

  /**
   * MLS group change
   *
   * **Trigger:** Participants join or leave the call
   * **Effect:** Updates MLS group membership and ratchets keys
   * **Security:** Maintains forward secrecy and post-compromise security
   */
  MlsGroupChange = "mls_group_change",
}
...
/**
 * DAVE Prepare Transition Data
 *
 * Payload data announcing the start of a protocol transition in E2EE calls.
 * All participants must prepare for the transition before it can be executed.
 *
 * **Coordination:** Clients must send DaveTransitionReady after preparing local state
 * for the transition.
 *
 * @see {@link https://daveprotocol.com/#protocol-transitions}
 */
export interface DavePrepareTransitionEntity {
   ...
   *
   * Identifies this specific transition across all participants.
   * Used to correlate preparation and execution messages.
   */
   ...
   *
   * Indicates what kind of protocol change is occurring.
   * Determines what preparation steps are required.
   */
  transition_type: DaveTransitionType;

  /**
   * Target protocol version (for protocol version changes)
   *
   * The DAVE protocol version that will be used after the transition.
   * Only present for protocol version change transitions.
   */
  target_protocol_version?: DaveProtocolVersion;
}

/**
 * DAVE Execute Transition Data
 *
 * Payload data signaling the execution of a protocol transition.
 * Sent when all participants are ready or when a timeout has been reached.
 *
 * **Effect:** Triggers the switch to the new protocol state across all participants.
 * Media senders can begin using the new protocol context after receiving this.
 */
export interface DaveExecuteTransitionEntity {
  /**
   * Transition identifier from the prepare transition
   *
   * Links this execution to the original preparation message.
   * Must match the transition_id from DavePrepareTransition.
   */
  transition_id: string;
}

/**
 * DAVE Transition Ready Data
 *
 * Payload data indicating readiness to execute a protocol transition.
 * Sent by clients after they have prepared their local state for the transition.
 *
 * **Coordination:** The transition will execute when all participants send this
 * message or when a timeout is reached.
 */
export interface DaveTransitionReadyEntity {
  /**
   * Transition identifier
   *
   * Identifies which transition this readiness signal applies to.
   * Must match the transition_id from the corresponding prepare message.
   */
  transition_id: string;
}

/**
 * DAVE Prepare Epoch Data
 *
 * Payload data announcing the preparation of a new MLS epoch for group updates.
 * Epochs represent distinct phases of the MLS group with different membership or keys.
 *
 * **Epoch 1:** Indicates a new MLS group is being created and participants need
 * to generate key packages.
 *
 * @see {@link https://daveprotocol.com/#mls-epochs}
 */
export interface DavePrepareEpochEntity {
  /**
   * Unique transition identifier
   *
   * Links this epoch preparation to the overall transition process.
   * Used for coordinating the epoch change across all participants.
   */
  transition_id: string;
   ...
   * Epoch identifier
   *
   * Sequential identifier for the MLS group epoch.
   * Epoch 1 indicates initial group creation.
   */
  epoch_id: number;

  /**
   * Protocol version for the epoch
   *
   * The DAVE protocol version that will be used in this epoch.
   * May change between epochs for protocol upgrades.
   */
 ...
 *
 * Payload data containing an MLS key package for group membership.
 * Key packages are cryptographic credentials needed to join an MLS group.
 *
 * **Usage:** Must be generated and sent when joining or rejoining an E2EE group.
 * Each key package is single-use and tied to a specific MLS group epoch.
 *
 * @see {@link https://daveprotocol.com/#key-packages}
 */
export interface DaveMlsKeyPackageEntity {
  /**
   * MLS key package binary data
   *
   * Serialized MLS KeyPackage containing the client's cryptographic credentials.
   * Format defined by the MLS specification (RFC 9420).
   */
  key_package: Uint8Array;
}

/**
 * DAVE MLS Commit Data
 *
 * Payload data containing an MLS commit message for group state changes.
 * Commits update the MLS group by applying pending proposals.
 *
 * **Group Changes:** Commits can add members, remove members, or update group keys.
 * Welcome messages are included when new members are added.
 *
 * @see {@link https://daveprotocol.com/#commits-and-proposals}
 */
export interface DaveMlsCommitEntity {
  /**
   * MLS commit binary data
   *
   * Serialized MLS Commit message updating the group state.
   * Contains the cryptographic operations to apply to the group.
   */
  commit: Uint8Array;
   ...
   *
   * MLS Welcome message for any members being added in this commit.
   * New members use this to join the group at the new epoch.
   */
  welcome?: Uint8Array;
}

/**
 * DAVE MLS External Sender Data
 *
 * Payload data containing an MLS external sender configuration.
 * The voice gateway must be an external sender to propose membership changes.
 *
 * **Purpose:** Allows the voice gateway to add and remove participants when they
 * join or leave the voice channel.
 */
export interface DaveMlsExternalSenderEntity {
  /**
   * External sender binary data
   *
   * Serialized MLS external sender public key and credential.
   * Used to verify proposals from the voice gateway.
   */
  external_sender: Uint8Array;
}

/**
 * DAVE MLS Proposals Data
 *
 * Payload data containing MLS proposals for group changes.
 * Proposals suggest changes to group membership that will be applied in commits.
 *
 * **Processing:** Participants must append or revoke received proposals before
 * generating commits for the epoch.
 *
 * @see {@link https://daveprotocol.com/#commits-and-proposals}
 */
export interface DaveMlsProposalsEntity {
  /**
   * Array of MLS proposal binary data
   *
   * Serialized MLS Proposal messages suggesting group changes.
   * Each proposal represents a single add or remove operation.
   */
  proposals: Uint8Array[];
}

/**
 * DAVE MLS Announce Commit Data
 *
 * Payload data announcing an MLS commit to all participants.
 * Contains the winning commit chosen by the voice gateway.
 *
 * **Selection:** The voice gateway broadcasts the first valid commit received
 * for each epoch, dropping later commits.
 */
export interface DaveMlsAnnounceCommitEntity {
  /**
   * MLS commit binary data
   *
   * The selected commit that will be applied to update the group.
   * All participants must apply this commit to progress to the next epoch.
   */
  commit: Uint8Array;
   ...
   *
   * The epoch number that the group will have after applying this commit.
   * Used to track group progression and detect out-of-order messages.
   */
  epoch_id: number;
}
 ...
 *
 * Payload data notifying about an invalid commit and providing recovery data.
 * Sent when a participant's group state becomes inconsistent.
 *
 * **Recovery:** The participant should reset their local state and rejoin using
 * the provided welcome message.
 */
export interface DaveMlsInvalidCommitWelcomeEntity {
  /**
   * New welcome message binary data
   *
   * MLS Welcome message for rejoining the group after state reset.
   * Participant should use this to establish a clean group state.
   */
  welcome: Uint8Array;

  /**
   * Reason for the invalid commit
   *
   * Human-readable description of why the commit was invalid.
   * Useful for debugging and understanding the failure.
   */
  reason?: string;
}
...
export interface VoiceReceivePayloads {
   ...
   * Signals the start of an E2EE protocol transition.
   */
  [VoiceGatewayOpcode.DavePrepareTransition]: DavePrepareTransitionEntity;

  /**
   * DAVE protocol transition execution signal.
   * Triggers the switch to new protocol state across all participants.
   */
  [VoiceGatewayOpcode.DaveExecuteTransition]: DaveExecuteTransitionEntity;

  /**
   * DAVE protocol epoch preparation for MLS group updates.
   * Announces new MLS epoch with protocol version information.
   */
  [VoiceGatewayOpcode.DavePrepareEpoch]: DavePrepareEpochEntity;

  /**
   * DAVE MLS commit announcement to all participants.
   * Contains the selected commit for group state updates.
   */
  [VoiceGatewayOpcode.DaveMlsAnnounceCommit]: DaveMlsAnnounceCommitEntity;

  /**
   * DAVE MLS invalid commit notification with recovery data.
   * Provides new welcome message when commit processing fails.
   */
...
export interface VoiceSendPayloads {
  ...
  /**
   * DAVE protocol transition readiness confirmation.
   * Indicates client is prepared for E2EE protocol transition.
   */
  [VoiceGatewayOpcode.DaveTransitionReady]: DaveTransitionReadyEntity;

  /**
   * DAVE MLS key package for group membership.
   * Required to join or rejoin an E2EE group.
   */
  [VoiceGatewayOpcode.DaveMlsKeyPackage]: DaveMlsKeyPackageEntity;

  /**
   * DAVE MLS commit message for group state changes.
   * Updates the MLS group by applying pending proposals.
   */
  [VoiceGatewayOpcode.DaveMlsCommit]: DaveMlsCommitEntity;

  /**
   * DAVE MLS external sender configuration.
   * Provides voice gateway external sender credentials.
   */
  [VoiceGatewayOpcode.DaveMlsExternalSender]: DaveMlsExternalSenderEntity;

  /**
   * DAVE MLS proposals for group changes.
   * Suggests changes to group membership for commit processing.
   */
